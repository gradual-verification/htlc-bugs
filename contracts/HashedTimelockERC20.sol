...
/**
    move to the latest version, 0.8.0 - 0.8.21
 */
...
contract HashedTimelockERC20 {
    ...
    modifier refundable(bytes32 _contractId) {
        ...
        // TODO: check why this is failing the transaction, (not to use now)
    ...
    /*@ 
        requires tokensTransferable(_tokenContract, msg.sender, _amount) && futureTimelock(_timelock)
     @*/

    /*@
        ensures ERC20(_tokenContract).balanceOf(address(this)) >= _amount && haveContract(contractId)
    @*/
    ...
    * @return bool true on success
    ...
    /*@ 
        requires acc(c.withdrawn) && 
                 acc(c.refunded) &&
                 acc(c.preimage) &&
                 ERC20(_tokenContract).balanceOf(address(this)) >= _amount &&
                 contractExists(_contractId) && hashlockMatches(_contractId, _preimage) && withdrawable(_contractId)
     @*/

    /*@
        ensures acc(c.withdrawn) && c.withdrawn = true &&
                acc(c.refunded) && c.refunded = false &&
                acc(c.preimage) && c.preimage = _preimage &&
                ERC20(_tokenContract).balanceOf(c.receiver) >= c.amount
    ...
    /*@ 
        requires acc(c.refunded) && 
                 acc(c.withdrawn) &&
                 contractExists(_contractId) && refundable(_contractId)
     @*/

    /*@
        ensures acc(c.refunded) && c.refunded = true &&
                acc(c.withdrawn) && c.withdrawn = false &&
                ERC20(_tokenContract).balanceOf(c.sender) >= c.amount
